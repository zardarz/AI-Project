...
public class GroundDiscipliner : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (stickmanGO.name == "Stick Man") { // if it is not the right or left shin and the top parent is called stick man
            stickmanGO.GetComponent<NeuralNetwork>().AddToScore(-100000); // we punish him a lot
        }
    }
}